...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        # The active WaDiff training loop does not use the legacy perceptual
        # loss. Build VGG19 lazily only if that legacy branch is requested.
        self.per_model = None
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, key=None,
    ):
        """
        ...
        assert t.shape == (B,)
        if key is not None:
            model_output = model((x, key), self._scale_timesteps(t), **model_kwargs)
        else:
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if ori_model_output is not None:
                ...
                if self.per_model is None:
                    ...
                    self.per_model = FeatureExtractor(
                        models.vgg19(weights=weights), 23
                    ).to(x_start.device).eval()
